...
void castRay(const vec3 ro, const vec3 rd, const vec3 inv_rd, in int max_iter, out Hit hit) {
	...
	vec3 t_delta = abs(max_size * inv_rd);
	bool skip = false;
	...
	stack[0].sub_coords = min(ivec3(stack[0].start_coords / max_size), 1);
	stack[0].t_max_min = 0;
	stack[0].t_max = ((dir + stack[0].sub_coords) * max_size - stack[0].start_coords) * inv_rd;
	...
	while (hit.iteration <= 1 << max_iter) {
		...
		bool is_empty = !bool(current_id & (0x1 << 8));
		bool is_leaf = !bool(current_id & 0x1);//voxels[context.index].child == 0;
		...
		if (is_leaf) {
			hit.inter = true;
			hit.id = voxels[voxels[context.index].child + sub_index].id >> 16;
			// Compute absolute hit point position
			//float total_t_max = 0;
			//for (int i=0; i<current_stack_index; i++) total_t_max += stack[i].t_max_min;

			//hit.point = stack[current_stack_index].start_coords + rd*(total_t_max-0.0001);
			return;
		}
		...
		stack[current_stack_index].sub_coords = min(ivec3(stack[current_stack_index].start_coords / current_size), 1);
		stack[current_stack_index].t_max_min = 0;
		stack[current_stack_index].t_max = ((dir + stack[current_stack_index].sub_coords) * current_size - stack[current_stack_index].start_coords) * inv_rd;
